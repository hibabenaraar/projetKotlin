...
class MaintenanceWorkersActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.maintenance_workers_layout)

        recyclerView = findViewById(R.id.maintenance_recycler_view)
    ...
    private fun fetchWorkers() {
        RetrofitClient.apiService.getWorkersByService("Maintenance")
            .enqueue(object : Callback<List<Worker>> {
                override fun onResponse(call: Call<List<Worker>>, response: Response<List<Worker>>) {
                    if (response.isSuccessful) {
                        ...
                        Toast.makeText(this@MaintenanceWorkersActivity, "Failed to load workers", Toast.LENGTH_SHORT).show()
                    }
                }

                override fun onFailure(call: Call<List<Worker>>, t: Throwable) {
                    Toast.makeText(this@MaintenanceWorkersActivity, "Error: ${t.message}", Toast.LENGTH_SHORT).show()
                }
            })
    }
        ...
        RetrofitClient.apiService.createWorker(worker)
            .enqueue(object : Callback<Worker> {
                override fun onResponse(call: Call<Worker>, response: Response<Worker>) {
                    if (response.isSuccessful) {
                        Toast.makeText(this@MaintenanceWorkersActivity, "Worker created successfully", Toast.LENGTH_SHORT).show()
                        fetchWorkers() // Refresh the list
                    } else {
                        Toast.makeText(this@MaintenanceWorkersActivity, "Failed to create worker", Toast.LENGTH_SHORT).show()
                    }
                }
    ...
    private fun updateWorker(worker: Worker) {
        RetrofitClient.apiService.updateWorker(worker.id, worker)
            .enqueue(object : Callback<Worker> {
                override fun onResponse(call: Call<Worker>, response: Response<Worker>) {
                    if (response.isSuccessful) {
                        Toast.makeText(this@MaintenanceWorkersActivity, "Worker updated successfully", Toast.LENGTH_SHORT).show()
                        fetchWorkers() // Refresh the list
                    } else {
                        Toast.makeText(this@MaintenanceWorkersActivity, "Failed to update worker", Toast.LENGTH_SHORT).show()
                    }
                }
        ...
        RetrofitClient.apiService.deleteWorker(worker.id)
            .enqueue(object : Callback<Void> {
                override fun onResponse(call: Call<Void>, response: Response<Void>) {
                    if (response.isSuccessful) {
                        Toast.makeText(this@MaintenanceWorkersActivity, "Worker deleted successfully", Toast.LENGTH_SHORT).show()
                        fetchWorkers()
                    } else {
                        Toast.makeText(this@MaintenanceWorkersActivity, "Failed to delete worker", Toast.LENGTH_SHORT).show()
                    }
                }
    ...
    private fun showWorkerDialog(isEdit: Boolean, worker: Worker? = null) {
        ...
        dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener {
            ...
            if (isEdit) {
                updateWorker(newWorker) // Corrected this line to update
            } else {
